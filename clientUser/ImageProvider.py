...
class ImageProvider:
    def __init__(self, pygame, path):
        ...
        self.load_images(path, pygame)

    def load_images(self, path, pygame):
            ...
            if properties[0] == 'car':
                if len(properties) == 3:
                    try:
                        ...
                        if version in self.car_images:
                            self.car_images[version].append((damage, image))
                        else:
                            self.car_images[version] = [(damage, image)]
                    except ValueError:
                ...
                if len(properties) == 3:
                    try:
                        ...
                        self.grass_images[(biome, version)] = image
                    except ValueError:
                ...
                if len(properties) == 4:
                    try:
                        ...
                        if (biome, version) in self.house_images:
                            self.house_images[(biome, version)].append((damage, image))
                        else:
                            self.house_images[(biome, version)] = [(damage, image)]
                    except ValueError:
                ...
                if len(properties) == 4:
                    try:
                        ...
                        if (biome, version) in self.street_images:
                            self.street_images[(biome, version)][directions] = image
                        else:
                            self.street_images[(biome, version)] = {directions: image}
                    except ValueError:
    ...
    def get_car(self, version=0, damage=0):
        d, image = self.find_closest(self.car_images[version], (damage,))
        return image

    def get_gras(self, biome=0, version=0):
        return self.gras_images[(biome, version)]

    def get_house(self, biome=0, version=0, damage=0):
        d, image = self.find_closest(self.house_images[(biome, version)], (damage,))
        return image
